...
class CouponIntegrationServiceTest {
    ...
    void create_validUser_Success() {
        CouponSaveResponse response = couponService.create(savedUser);

        Assertions.assertThat(response)
                .hasFieldOrPropertyWithValue("userId", savedUser.getId())
                .hasFieldOrPropertyWithValue("name", savedUser.getNickname() + "의 쿠폰")
    ...
    void getCouponList_validUser_Success() {
        couponService.create(savedUser);

        CouponSearchResponses couponList = couponService.getCouponList(savedUser);
